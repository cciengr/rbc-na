...
function AccordionList({ items, accentEmail }: { items: { q: string; a: string }[]; accentEmail?: string }) {
  const { t } = useTheme();
    ...
    <div className="flex flex-col gap-2">
      {items.map((faq, i) => (
        <div key={i} className="rounded-xl overflow-hidden transition-all duration-200"
          style={{ backgroundColor: openIndex === i ? t.faqOpenBg : t.faqClosedBg, border: `1px solid ${openIndex === i ? t.faqOpenBorder : t.faqClosedBorder}` }}>
...
export function FAQSection() {
  const { t } = useTheme();
  const { faqs, childcare, site } = useSiteContent();
  const [activeTab, setActiveTab] = useState<"general" | "childcare">("general");
